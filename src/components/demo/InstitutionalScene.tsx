    ...
    <div className="h-full flex flex-col items-center justify-center p-8">
      ...
      >
        {stats.map((stat, idx) => (
          ...
          >
            ...
            <p className="text-2xl font-semibold text-gradient-livemed mb-1">{stat.value}</p>
      ...
      >
        <motion.div
          ...
          whileTap={{ scale: 0.98 }}
          className="btn-glow gradient-livemed text-white px-8 py-4 rounded-full flex items-center gap-2 font-medium cursor-pointer"
        >
          <span>Start Your Journey</span>
          <ArrowRight className="h-4 w-4" />
        </motion.div>
        <p className="text-xs text-white/40">No credit card required • Free trial available</p>
      </motion.div>
